...
class Media {
  ...
  onResize({ screen, viewport } = {}) {
    ...
    let scaleFactor;
    let aspectRatio = 0.8; // Default aspect ratio (width/height)
    ...
    // Calculate card dimensions relative to viewport
    const cardHeight = this.viewport.height * scaleFactor;
    ...
    // Responsive padding
    this.padding = screenWidth < 768 ? 0.8 : (screenWidth < 1024 ? 1.5 : 2);
...
class App {
  ...
  onTouchDown(e) {
    this.isDown = true;
    this.isDragging = false;
    this.clickStartedOnCanvas = e.target === this.gl.canvas;
  ...
  onTouchUp() {
    ...
    const clickStartedOnCanvas = this.clickStartedOnCanvas;
    
    this.isDown = false;
    
    // Only trigger click if it wasn't a drag AND the click started on the canvas
    if (wasDown && !this.isDragging && this.onCardClick && clickStartedOnCanvas) {
      this.handleClick();
    }
  ...
  handleClick() {
    ...
    this.medias.forEach(media => {
      ...
      const cardCenterX = (this.screen.width / 2) + (media.plane.position.x * (this.screen.width / this.viewport.width));
      const distance = Math.abs(cardCenterX - centerX);
      
      if (distance < closestDistance && distance < media.plane.scale.x * (this.screen.width / this.viewport.width) / 2) {
        closestDistance = distance;
        closestMedia = media;
      }
    });
    
    if (closestMedia && closestMedia.episodeId && this.onCardClick) {
      this.onCardClick(closestMedia.episodeId);
    }
  }
  onResize() {
    this.screen = {
  ...
  addEventListeners() {
    ...
    // Keep these on window but check if target is canvas
...
export default function CircularGallery({
  ...
  useEffect(() => {
    const app = new App(containerRef.current, { items, bend, textColor, borderRadius, font, scrollSpeed, scrollEase, onCardClick });
    return () => {
      app.destroy();
    };
